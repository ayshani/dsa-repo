...
The master server will process all newly arrived messages instantly and send a reply to the originating
server via the reversed path the message had gone through.

At the beginning of second 0, each data server sends its message to be processed. Starting from second 1,
at the beginning of every second, each data server will check if it has received a reply to the message
...
every patience[i] second(s), i.e., the data server i will resend the message if patience[i] second(s) have
elapsed since the last time the message was sent from this server.
...
TC : o(E + n) where E = no. of edges, n = no. of Nodes
SC : o(E + n)
 */
    ...
    public int networkBecomesIdle(int[][] edges, int[] patience) {
        ...
        while(!q.isEmpty()){
            ...
            while(size-->0){
                ...
                for(int next : adjGraph.get(node)){
                    if(visited[next]==-1){
                        q.offer(next);
        ...
        for(int i=1;i<n;i++){
            int timeTotravelForAMessage = visited[i];
            int totalTime = timeTotravelForAMessage+ timeTotravelForAMessage - (timeTotravelForAMessage%patience[i]);

            if(timeTotravelForAMessage%patience[i]==0)
                totalTime-=patience[i];

            MaxWorkTime = Math.max(MaxWorkTime, totalTime);
        }

        return MaxWorkTime+1;
    }
}
