...
class StockPrice {
    ...
    TreeMap<Integer, Set<Integer>>  priceVal;
    public StockPrice() {
        map = new TreeMap<>();
        priceVal = new TreeMap<>();
    }

    public void update(int timestamp, int price) {
        if(map.containsKey(timestamp)){
            int previousPrice = map.get(timestamp);
            Set<Integer> set = priceVal.get(previousPrice);
            set.remove(timestamp);
            if(set.isEmpty()){
                priceVal.remove(previousPrice);
            }
        }
        priceVal.putIfAbsent(price, new HashSet<Integer>());
        priceVal.get(price).add(timestamp);
    ...
    public int maximum() {
        return priceVal.lastKey();
    }

    public int minimum() {
        return priceVal.firstKey();
    }
}
