...
public class GroupThePeopleGivenTheGroupSizeTheyBelongTo {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(new GroupThePeopleGivenTheGroupSizeTheyBelongTo().groupThePeople(group));
    }
    public List<List<Integer>> groupThePeople(int[] groupSizes) {
        ...
        for(int i=0;i<groupSizes.length;i++){
            sizeToGroup.computeIfAbsent(groupSizes[i], value -> new ArrayList<>()).add(i);
            if(sizeToGroup.get(groupSizes[i]).size()== groupSizes[i]){
                result.add(sizeToGroup.get(groupSizes[i]));
